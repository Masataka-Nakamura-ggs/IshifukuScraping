...
class TestScrapeGoldPrice:
    ...
    def test_scrape_gold_price_success(
        self, mock_soup, mock_wait, mock_chrome, mock_driver_manager
    ):
        """スクレイピング成功のテスト"""
        # モックの設定
    ...
    @patch("scrape_ishifuku.WebDriverWait")
    @patch("scrape_ishifuku.BeautifulSoup")
    def test_scrape_gold_price_no_data_found(
        self, mock_soup, mock_wait, mock_chrome, mock_driver_manager
    ):
        """価格データが見つからない場合のテスト"""
        # モックの設定
    ...
    def test_scrape_gold_price_fallback_search(
        self, mock_findall, mock_soup, mock_wait, mock_chrome, mock_driver_manager
    ):
        """フォールバック検索のテスト"""
        # モックの設定
        mock_driver = MagicMock()
        ...
        mock_driver.page_source = (
            "<html><body><p>Gold price: 18,500 yen</p></body></html>"
        )
...
class TestSeleniumIntegration:
    ...
    def test_main_function_success(
        self, mock_save_csv, mock_get_datetime, mock_setup_logging, mock_scrape
    ):
        """main関数の成功テスト"""
        # モックの設定
        mock_get_datetime.return_value = (
            "2025-08-21",
            "2025-08-21 15:30:45",
            "20250821",
        )
